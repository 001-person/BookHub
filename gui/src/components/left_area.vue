<template>
  ...
  export default {
    ...
    async mounted() {
      ...
      eventBus.on('add_mark', (markid) => {
        this.addMark(markid);
    });

      eventBus.on('update:delete_chapter', (chapter_id) => {
        this.handle_c_v_delete(chapter_id);
        
    });

      eventBus.on('update:delete_volume', (volume_id) => {
        this.handle_c_v_delete(volume_id);
