<template>
  ...
  import { intersection, waitForPywebviewApi} from '../customFuncs.js'
    ...
    components: {
      ...
      data() {
        return {
          ...
          book_path:'D:/node_project/BookHub/BookHub/mybooks',
          books_surface_data:[],
          tags_to_books:[],
      ...
      async mounted() {
        ...
        eventBus.on('update_book_info',(new_book_info)=>{
          this.update_bookInfo(new_book_info);

          });

        eventBus.on('new_book_data',(new_book_info)=>{
          this.isopen = false;
          this.create_book_done(new_book_info);

          });

        eventBus.on('delete-book',(book_id)=>{
            this.delete_book(book_id);

            });

        eventBus.on('simple-search',(search_data)=>{

          this.simple_search(search_data);
        });

        eventBus.on('filt-search',(search_form)=>{
          this.filt_search(search_form);
        });

        
          // 跳转到写作界面
          eventBus.on('to_write_view',(book_id)=>{
        ...
        // 离开写作页面时，保存书籍信息
        eventBus.on('save-book-info',(bookdata)=>{
