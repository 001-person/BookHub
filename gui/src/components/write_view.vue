...
export default {
  ...
  data() {
    return {
      ...
      bookpath:'D:/node_project/BookHub/BookHub/mybooks',
  ...
  async mounted() {
    ...
    eventBus.on('update:current_node_key', (keys) => {
    ...
    eventBus.on('update:new_chapter', (new_id) => {
      this.new_chapter(new_id);
    });

    eventBus.on('update:new_volume', (new_id) => {
      this.new_volume(new_id);
    });

    eventBus.on('update:delete_chapter', (delete_id) => {
      this.delete_chapter(delete_id);
    });

    eventBus.on('update:delete_volume', (delete_id) => {
      this.delete_volume(delete_id);
    });

    eventBus.on('update:rename_node', (data) => {
      this.rename_node(data);
    });

    eventBus.on('remove_mark', (markid) => {
      this.remove_mark(markid);
    });

    eventBus.on('jump_to_mark', (markid) => {
